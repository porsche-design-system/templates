...
const getAiButton = (page: Page) =>
  page.getByRole("button", { name: "open / close ai assistant" });
const getLikeButton = (page: Page) =>
  page.getByRole("button", { name: "Like" }).first();
const getDislikeButton = (page: Page) =>
  page.getByRole("button", { name: "Dislike" }).first();
const getCopyToClipboardButton = (page: Page) =>
...
test.describe("has WCAG 2.2 (AA) compliance", () => {
  test.describe('for "login"', () => {
    ...
    test('ai button exposes "aria-expanded"', async ({ page, browserName }) => {
      test.skip(
        browserName === "webkit",
        "Not possible to click on button in open state on mobile",
      );
      await expect(getAiButton(page)).toMatchAriaSnapshot(
        '- button "open / close ai assistant" [expanded]',
      );

      await getAiButton(page).click();
      await expect(getAiButton(page)).toMatchAriaSnapshot(
        '- button "open / close ai assistant"',
      );
  ...
  test.describe('for "chat"', () => {
    ...
    test('like + dislike button exposes "aria-pressed"', async ({ page }) => {
      await expect(getDislikeButton(page)).toMatchAriaSnapshot(
        '- button "Dislike"',
      );
      ...
      await expect(getLikeButton(page)).toMatchAriaSnapshot(
        '- button "Like" [pressed]',
      );

      await getDislikeButton(page).click();
      await expect(getDislikeButton(page)).toMatchAriaSnapshot(
        '- button "Dislike" [pressed]',
      );
      await expect(getLikeButton(page)).toMatchAriaSnapshot('- button "Like"');
    ...
    test('copy to clipboard button exposes "aria-pressed"', async ({
      page,
    }) => {
      await expect(getCopyToClipboardButton(page)).toMatchAriaSnapshot(
